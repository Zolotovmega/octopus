...
describe Octopus::Model do
  describe '#using method' do
    it 'raise when Model#using receives a block' do
      expect { User.using_shard(:master) { true } }.to raise_error(Octopus::Exception, /User\.using is not allowed to receive a block/)
    end

    it 'should allow to send a block to the master shard' do
      Octopus.using_shard(:master) do
        User.create!(:name => 'Block test')
      end

      expect(User.using_shard(:master).find_by_name('Block test')).not_to be_nil
    end

    it 'should allow to pass a string as the shard name to a AR subclass' do
      User.using_shard('canada').create!(:name => 'Rafael Pilha')

      expect(User.using_shard('canada').find_by_name('Rafael Pilha')).not_to be_nil
    end

    it 'should allow comparison of a string shard name with symbol shard name' do
      u = User.using_shard('canada').create!(:name => 'Rafael Pilha')
      expect(u).to eq(User.using_shard(:canada).find_by_name('Rafael Pilha'))
    end
    ...
    it 'should allow selecting the shards on scope' do
      User.using_shard(:canada).create!(:name => 'oi')
      expect(User.using_shard(:canada).count).to eq(1)
      expect(User.count).to eq(0)
    end

    it 'should allow selecting the shard using #new' do
      u = User.using_shard(:canada).new
      u.name = 'Thiago'
      u.save

      expect(User.using_shard(:canada).count).to eq(1)
      expect(User.using_shard(:brazil).count).to eq(0)

      u1 = User.new
      u1.name = 'Joaquim'
      u2 = User.using_shard(:canada).new
      u2.name = 'Manuel'
      u1.save
      u2.save

      expect(User.using_shard(:canada).all).to eq([u, u2])
      expect(User.all).to eq([u1])
    end

    describe 'multiple calls to the same scope' do
      it 'works with nil response' do
        scope = User.using_shard(:canada)
        expect(scope.count).to eq(0)
        expect(scope.first).to be_nil
      end

      it 'works with non-nil response' do
        user = User.using_shard(:canada).create!(:name => 'oi')
        scope = User.using_shard(:canada)
    ...
    it 'should allow creating more than one user' do
      User.using_shard(:canada).create([{ :name => 'America User 1' }, { :name => 'America User 2' }])
      User.create!(:name => 'Thiago')
      expect(User.using_shard(:canada).find_by_name('America User 1')).not_to be_nil
      expect(User.using_shard(:canada).find_by_name('America User 2')).not_to be_nil
      expect(User.using_shard(:master).find_by_name('Thiago')).not_to be_nil
    end

    it 'should work when you have a SQLite3 shard' do
      u = User.using_shard(:sqlite_shard).create!(:name => 'Sqlite3')
      expect(User.using_shard(:sqlite_shard).where(name: 'Sqlite3').first).to eq(u)
    end

    it 'should clean #current_shard from proxy when using execute' do
      User.using_shard(:canada).connection.execute('select * from users limit 1;')
    ...
    it 'should allow scoping dynamically' do
      User.using_shard(:canada).using_shard(:master).using_shard(:canada).create!(:name => 'oi')
      expect(User.using_shard(:canada).using_shard(:master).count).to eq(0)
      expect(User.using_shard(:master).using_shard(:canada).count).to eq(1)
    end

    it 'should allow find inside blocks' do
      @user = User.using_shard(:brazil).create!(:name => 'Thiago')

      Octopus.using_shard(:brazil) do
        expect(User.first).to eq(@user)
      end

      expect(User.using_shard(:brazil).find_by_name('Thiago')).to eq(@user)
    end
    ...
    it 'should support both groups and alone shards' do
      _u = User.using_shard(:alone_shard).create!(:name => 'Alone')
      expect(User.using_shard(:alone_shard).count).to eq(1)
      expect(User.using_shard(:canada).count).to eq(0)
      expect(User.using_shard(:brazil).count).to eq(0)
      expect(User.count).to eq(0)
    end

    it 'should work with named scopes' do
      u = User.using_shard(:brazil).create!(:name => 'Thiago')

      expect(User.thiago.using_shard(:brazil).first).to eq(u)
      expect(User.using_shard(:brazil).thiago.first).to eq(u)
    ...
    describe '#current_shard attribute' do
      it 'should store the attribute when you create or find an object' do
        u = User.using_shard(:alone_shard).create!(:name => 'Alone')
        expect(u.current_shard).to eq(:alone_shard)
        User.using_shard(:canada).create!(:name => 'oi')
        u = User.using_shard(:canada).find_by_name('oi')
        expect(u.current_shard).to eq(:canada)
      end

      it 'should store the attribute when you find multiple instances' do
        5.times { User.using_shard(:alone_shard).create!(:name => 'Alone') }

        User.using_shard(:alone_shard).all.each do |u|
          expect(u.current_shard).to eq(:alone_shard)
        end
      end

      it 'should works when you find, and after that, alter that object' do
        alone_user = User.using_shard(:alone_shard).create!(:name => 'Alone')
        _mstr_user = User.using_shard(:master).create!(:name => 'Master')
        alone_user.name = 'teste'
        alone_user.save
        expect(User.using_shard(:master).first.name).to eq('Master')
        expect(User.using_shard(:alone_shard).first.name).to eq('teste')
      end

      it 'should work for the reload method' do
        User.using_shard(:alone_shard).create!(:name => 'Alone')
        u = User.using_shard(:alone_shard).find_by_name('Alone')
        u.reload
    ...
    describe 'passing a block' do
      it 'should allow queries be executed inside the block, ponting to a specific shard' do
        ...
        expect(User.using_shard(:canada).count).to eq(1)
        expect(User.using_shard(:master).count).to eq(0)
        expect(User.count).to eq(0)
      end
    ...
    describe 'raising errors' do
      it "should raise a error when you specify a shard that doesn't exist" do
        expect { User.using_shard(:crazy_shard).create!(:name => 'Thiago') }.to raise_error('Nonexistent Shard Name: crazy_shard')
      end
    end
    ...
    describe 'equality' do
      ...
      it 'should work with persisted objects' do
        u = User.using_shard(:brazil).create(:name => 'Mike')
        expect(User.using_shard(:brazil).find_by_name('Mike')).to eq(u)
      end
      ...
      it 'delegates equality check on scopes' do
        u = User.using_shard(:brazil).create!(:name => 'Mike')
        expect(User.using_shard(:brazil).where(:name => 'Mike')).to eq([u])
      end
    end
  end
  ...
  describe 'using a postgresql shard' do
    it 'should update the Arel Engine' do
      expect(User.using_shard(:postgresql_shard).arel_engine.connection.adapter_name).to eq('PostgreSQL')
      expect(User.using_shard(:alone_shard).arel_engine.connection.adapter_name).to eq('Mysql2')
    end

    it 'should works with writes and reads' do
      u = User.using_shard(:postgresql_shard).create!(:name => 'PostgreSQL User')
      expect(User.using_shard(:postgresql_shard).all).to eq([u])
      expect(User.using_shard(:alone_shard).all).to eq([])
  ...
  describe 'AR basic methods' do
    ...
    it 'increment' do
      _ = User.using_shard(:brazil).create!(:name => 'Teste', :number => 10)
      u = User.using_shard(:brazil).find_by_number(10)
      u.increment(:number)
      u.save
      expect(User.using_shard(:brazil).find_by_number(11)).not_to be_nil
    end
    ...
    it 'decrement' do
      ...
      u.decrement(:number)
      u.save
      expect(User.using_shard(:brazil).find_by_number(9)).not_to be_nil
    end
    ...
    it 'toggle' do
      _ = User.using_shard(:brazil).create!(:name => 'Teste', :admin => false)
      u = User.using_shard(:brazil).find_by_name('Teste')
      u.toggle(:admin)
      u.save
      expect(User.using_shard(:brazil).find_by_name('Teste').admin).to be true
    end
    ...
    it 'count' do
      _u = User.using_shard(:brazil).create!(:name => 'User1')
      _v = User.using_shard(:brazil).create!(:name => 'User2')
      _w = User.using_shard(:brazil).create!(:name => 'User3')
      expect(User.using_shard(:brazil).where(:name => 'User2').all.count).to eq(1)
    end

    it 'maximum' do
      _u = User.using_shard(:brazil).create!(:name => 'Teste', :number => 11)
      _v = User.using_shard(:master).create!(:name => 'Teste', :number => 12)

      expect(User.using_shard(:brazil).maximum(:number)).to eq(11)
      expect(User.using_shard(:master).maximum(:number)).to eq(12)
    end

    describe 'any?' do
      before { User.using_shard(:brazil).create!(:name => 'User1') }

      it 'works when true' do
        scope = User.using_shard(:brazil).where(:name => 'User1')
        expect(scope.any?).to be true
      end

      it 'works when false' do
        scope = User.using_shard(:brazil).where(:name => 'User2')
        expect(scope.any?).to be false
      end
    end

    it 'exists?' do
      @user = User.using_shard(:brazil).create!(:name => 'User1')

      expect(User.using_shard(:brazil).where(:name => 'User1').exists?).to be true
      expect(User.using_shard(:brazil).where(:name => 'User2').exists?).to be false
    end

    describe 'touch' do
      it 'updates updated_at by default' do
        @user = User.using_shard(:brazil).create!(:name => 'User1')
        User.using_shard(:brazil).where(:id => @user.id).update_all(:updated_at => Time.now - 3.months)
        @user.touch
      ...
      it 'updates passed in attribute name' do
        @user = User.using_shard(:brazil).create!(:name => 'User1')
        User.using_shard(:brazil).where(:id => @user.id).update_all(:created_at => Time.now - 3.months)
    ...
    describe '#pluck' do
      ...
      it 'should works from scope proxy' do
        names = User.using_shard(:brazil).pluck(:name)
        expect(names).to eq(['User1'])
        expect(User.using_shard(:master).pluck(:name)).to eq([])
      end
    end

    it 'update_column' do
      @user = User.using_shard(:brazil).create!(:name => 'User1')
      @user2 = User.using_shard(:brazil).find(@user.id)
      @user2.update_column(:name, 'Joaquim Shard Brazil')
      expect(User.using_shard(:brazil).find_by_name('Joaquim Shard Brazil')).not_to be_nil
    end
    ...
    it 'update_attributes' do
      ...
      expect(User.using_shard(:brazil).find_by_name('Joaquim')).not_to be_nil
    end
    ...
    it 'as_json' do
      ...
      user = User.using_shard(:brazil).where(:name => 'User1').first
    ...
    it 'transaction' do
      ...
      expect(User.using_shard(:brazil).count).to eq(0)
      expect(User.using_shard(:master).count).to eq(1)

      User.using_shard(:brazil).transaction do
        expect(User.find_by_name('Thiago')).to be_nil
        User.create!(:name => 'Brazil')
      end

      expect(User.using_shard(:brazil).count).to eq(1)
      expect(User.using_shard(:master).count).to eq(1)
    end
    ...
    describe 'deleting a record' do
      ...
      it 'delete' do
        @user2.delete
        expect { User.using_shard(:brazil).find(@user2.id) }.to raise_error(ActiveRecord::RecordNotFound)
      end
  ...
  describe 'custom connection' do
    context 'by default' do
      ...
      it 'should ignore using called on relation' do
        expect(CustomConnection.using_shard(:postgresql_shard).connection.current_database).to eq('octopus_shard_2')
      end

      it 'should ignore Octopus.using block' do
        Octopus.using_shard(:postgresql_shard) do
          expect(CustomConnection.connection.current_database).to eq('octopus_shard_2')
        end
      end
    ...
    context 'with allowed_shards configured' do
      ...
      it 'with using called on relation with allowed shard should use' do
        expect(CustomConnection.using_shard(:postgresql_shard).connection.current_database).to eq('octopus_shard_1')
      end
      ...
      it 'with using called on relation with disallowed shard should not use' do
        expect(CustomConnection.using_shard(:brazil).connection.current_database).to eq('octopus_shard_2')
      end
      ...
      it 'should clean up correctly' do
        ...
        CustomConnection.using_shard(:postgresql_shard).first
      ...
      it 'should clean up correctly even inside block' do
        ...
        Octopus.using_shard(:master) do
          CustomConnection.using_shard(:postgresql_shard).connection.execute('select count(*) from users')
          expect(User.first).not_to be_nil
        end
      end
    end
  ...
  describe 'when using set_table_name' do
    it 'should work correctly' do
      Bacon.using_shard(:brazil).create!(:name => 'YUMMMYYYY')
    end

    it 'should work correctly with a block' do
      Cheese.using_shard(:brazil).create!(:name => 'YUMMMYYYY')
    end
  end

  describe 'when using table_name=' do
    it 'should work correctly' do
      Ham.using_shard(:brazil).create!(:name => 'YUMMMYYYY')
    end
  end
  ...
  describe 'when using a environment with a single adapter' do
    it 'should not clean the table name' do
      OctopusHelper.using_environment :production_fully_replicated do
        ...
        Keyboard.using_shard(:master).create!(:name => 'Master Cat')
      end
    end
  end
  ...
  describe 'when you have joins/include' do
    before(:each) do
      @client1 = Client.using_shard(:brazil).create(:name => 'Thiago')
      ...
      @item4 = Item.using_shard(:brazil).create(:client => @client1, :name => 'Item 4')
    end

    it 'should work using the rails 3.x syntax' do
      items = Item.using_shard(:canada).joins(:client).where("clients.id = #{@client2.id}").all
      expect(items).to eq([@item1, @item2])
    end

    it 'should work for include also, rails 3.x syntax' do
      items = Item.using_shard(:canada).includes(:client).where(:clients => { :id => @client2.id }).all
      expect(items).to eq([@item1, @item2])
    end
  end
  ...
  describe 'ActiveRecord::Base Validations' do
    it 'should work correctly when using validations' do
      ...
      expect { Keyboard.using_shard(:brazil).create!(:name => 'Key') }.not_to raise_error
      expect { Keyboard.create!(:name => 'Key') }.to raise_error
    end

    it 'should work correctly when using validations with using syntax' do
      @key = Keyboard.using_shard(:brazil).create!(:name => 'Key')
      expect { Keyboard.create!(:name => 'Key') }.not_to raise_error
      expect { Keyboard.using_shard(:brazil).create!(:name => 'Key') }.to raise_error
    end
  end
